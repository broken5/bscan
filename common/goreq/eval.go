...
func NewEnvOption() CustomLib {
	...
	c.envOptions = []cel.EnvOption{
		...
		cel.Declarations(
			...
			decls.NewIdent("aw", decls.NewObjectType("goreq.AliveWeb"), nil),
			decls.NewIdent("response", decls.NewObjectType("goreq.Response"), nil),
...
// }
func ExecuteExpr(args map[string]interface{}) func(string) (ref.Val, error) {
	c := NewEnvOption()
	env, err := NewEnv(&c)
	if err != nil {
		return nil
	...
	return func(expr string) (ref.Val, error) {
		return Evaluate(env, expr, variableMap)
	}
}
